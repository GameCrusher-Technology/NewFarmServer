...
class UserEventLogManager extends ManagerBase {
	...
	public function writeLog($action, $params = null) {
		$this->autoCommit();
		switch($action){
			case ActionCode::ACTION_BUY:
			case ActionCode::ACTION_SELL:
			case ActionCode::ACTION_EXTEND:
			case ActionCode::ACTION_SWITCH_ITEM:
			case ActionCode::ACTION_FINISH_BUILD_HOUSE:
			case ActionCode::ACTION_COMPLETE_JOB:
			case ActionCode::ACTION_FINISH_RAISE_MACHINE:
			case ActionCode::ACTION_SEND_GIFT:
			case ActionCode::ACTION_PROCESS:
			case ActionCode::ACTION_HARVEST_PROCESS:
			case ActionCode::ACTION_VISIT_FRIENDS:
			case ActionCode::ACTION_GET_CHRISTMAS_GIFT:
			case ActionCode::ACTION_OPEN_BOX:
			case ActionCode::ACTION_ITEM_LIB_HARVEST:
			case ActionCode::ACTION_GET_DAILY_REWARD:
			case ActionCode::ACTION_GET_EASTER_REWARD:
			case ActionCode::ACTION_HIT_BIG_EGG:
			case ActionCode::ACTION_GET_HIT_BIG_EGG_REWARD:
			case ActionCode::ACTION_ACCEPT_GIFT:
			case ActionCode::ACTION_BEAUTY_CONTEST:
			case ActionCode::ACTION_UPGRADE_LEVEL:
			case ActionCode::ACTION_ADD_VIP_REWARD:
			case ActionCode::ACTION_ADD_VIP_NEW_USER_REWARD:
			case ActionCode::ACTION_CALL_BACK_PET:
			case ActionCode::ACTION_PET_PLAY:
			case ActionCode::ACTION_ADD_ROOM_LIB:
			case ActionCode::ACTION_DELETE_ROOM:
			case ActionCode::ACTION_DELETE_ROOM_ITEM:
			case ActionCode::ACTION_USE_ROOM:
			case ActionCode::ACTION_BUY_TASK_NODE:
			case ActionCode::ACTION_BUY_TASK:
			case ActionCode::ACTION_EXCHANGE_COLLECTION:
			case ActionCode::ACTION_CLEAN_PET:
			case ActionCode::ACTION_ADD_HOUSE_CARD:
			case ActionCode::ACTION_GET_EXCHANGEGOODS:
			case ActionCode::ACTION_GET_FRIEND_SWEETS:
			case ActionCode::ACTION_GET_EXCHANGE_SWEETS:
			case ActionCode::ACTION_GET_DECO_REWARDS:
			case ActionCode::ACTION_GET_RANK_REWARD:
			case ActionCode::ACTION_XMAS_REFRESH_LOTTERY:
			case ActionCode::ACTION_HIT_MONKEY:									
				$this->writeUseItemEvent($action,$params);
				break;
			case ActionCode::ACTION_STEAL_CATCH:
				$this->writeStealCaughtEvent($params['uid'], $params['name'], $params['coin']);
				break;
			case ActionCode::ACTION_FRIEND_STEAL:
				$this->writeStealEvent($params['uid'], $params['name'], $params['item_id'], $params['item_count']);
				break;
			case ActionCode::ACTION_EVENT_DO_BAD:
			case ActionCode::ACTION_EVENT_DO_HELP:
			case ActionCode::ACTION_FRIEND_FERTILIZE:
				$this->writeFriendEventLog($action, $params);
				break;
			default:
	...
	private function writeStealCaughtEvent($uid, $name, $coin) {
		$now = time();
		if(isset($this->log[KEY_CURRENT_EVENT_LOG][ActionCode::ACTION_STEAL_CATCH][$uid])){
			$catch_log = &$this->log[KEY_CURRENT_EVENT_LOG][ActionCode::ACTION_STEAL_CATCH][$uid];
			$catch_log['coin'] += $coin;
			if(!isset($catch_log['name'])){
				$catch_log['name'] = $name;
			}
		}
		else{
			$this->log[KEY_CURRENT_EVENT_LOG][ActionCode::ACTION_STEAL_CATCH][$uid] = array('coin' => $coin, 'name' => $name, 'time' => time());
		}
		$this->log[KEY_LAST_MERGE_TIME] = $now;
	}

	private function writeStealEvent($uid, $name, $item_id, $item_count) {
		$now = time();
		if(isset($this->log[KEY_CURRENT_EVENT_LOG][ActionCode::ACTION_FRIEND_STEAL][$uid])){
			$steal_log = &$this->log[KEY_CURRENT_EVENT_LOG][ActionCode::ACTION_FRIEND_STEAL][$uid];
			if(isset($steal_log[$item_id])){
				$steal_log[$item_id] += $item_count;
			}
			else{
				$steal_log[$item_id] = $item_count;
			}
			if(!isset($steal_log['name'])){
				$steal_log['name'] = $name;
			}
		} else{
			$this->log[KEY_CURRENT_EVENT_LOG][ActionCode::ACTION_FRIEND_STEAL][$uid] = array('time' => time(), $item_id => $item_count, 'name' => $name);
		}
		$this->log[KEY_LAST_MERGE_TIME] = $now;
	}
...
class EventLogFormatter {
	public function format($action, $events){
		switch($action){
			case ActionCode::ACTION_FRIEND_STEAL:
				$action_log = $this->formatStealEvent($events);
				break;
			case ActionCode::ACTION_BUY:
			case ActionCode::ACTION_SELL:
			case ActionCode::ACTION_EXTEND:
			...
			case ActionCode::ACTION_XMAS_REFRESH_LOTTERY:
			case ActionCode::ACTION_HIT_MONKEY:											
				$action_log = $this->formatItemEvent($action, $events);
				break;
			case ActionCode::ACTION_STEAL_CATCH:
				$action_log = $this->formatStealCaughtEvent($events);
				break;
			case ActionCode::ACTION_EVENT_DO_BAD:
			case ActionCode::ACTION_EVENT_DO_HELP:
				$action_log = $this->formatFriendEvent($action, $events);
				break;
			default:
	 ...
	 * @return array
	 */
	protected function formatStealEvent($events) {
		$log = array();
		foreach($events as $uid => $items){
			$time = $items['time'];
			unset($items['time']);
			$item_msg = $this->formatItemMsg($items);
			if(empty($items['name'])){
				$msg = sprintf("%d|%s", $uid,$item_msg);
			}
			else{
				$msg = sprintf("%d|%s|%s", $uid, $items['name'], $item_msg);
			}
			$log[] = array('action' => ActionCode::ACTION_FRIEND_STEAL, 'content' => $msg, 'create_time' => $time);
		}
		return $log;
	}
	 ...
	 * @return array
	 */
	protected function formatStealCaughtEvent($events){
		if(empty($events)){
			return '';
		}
		$log = array();
		foreach($events as $uid => $event){
			...
			if(empty($event['name'])){
				$msg = sprintf('%d|%d', $uid, $event['coin']);
			}
			else{
				$msg = sprintf('%d|%s|%d', $uid, $event['name'], $event['coin']);
			}
			$log[] = array('action' => ActionCode::ACTION_STEAL_CATCH,
			'content' => $msg,'create_time' => $event['time']);
		}
		return $log;
	}
}
