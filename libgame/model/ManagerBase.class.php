...
abstract class ManagerBase {
	...
	protected function insertDB($change, $cache_type = null){
		...
		$req=RequestFactory::createInsertRequest(get_app_config($change['gameuid']));
		$req->setKey('gameuid',$change['gameuid']);
		$req->setTable($this->getTableName());
	...
	protected function insertDBBatch($change,$cache_type = null){
		...
		foreach($change as $ch){
			$req=RequestFactory::createInsertRequest(get_app_config($ch['gameuid']));
			$req->setTable($this->getTableName());
			$req->setKey('gameuid',$ch['gameuid']);
