 ...
 * @param  $count
 */
function addSystemStat($col,$count = 1){
	require_once APP_ROOT.'/libgame/extends/SystemStatistics.class.php';
	$SysStat = new SystemStatistics();
	return $SysStat->addCount($col,$count);
}


function addRetentionStat($type,$level,$count=1){
	require_once APP_ROOT.'/libgame/extends/SystemStatistics.class.php';
	$SysStat = new SystemStatistics();
	return $SysStat->addRetentionCount($type,$level,$count);
}
function addShopStat($item_id,$count=1,$buy_method='c'){
	require_once APP_ROOT.'/libgame/extends/ShopStatistics.class.php';
	$ShopStat = new ShopStatistics();
	return $ShopStat->addItemCount($item_id,$count,$buy_method);
}
