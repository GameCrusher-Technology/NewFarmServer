...
$payLog = LogFactory::getLogger(array(
			...
			'log_dir' => APP_ROOT.'/log/91payment/', // 文件所在的目录
...
try{
	$Res = pay_result_notify_process($MyAppId,$MyAppKey);
			...
			// 其他的错误
	$payLog->writeError("database exception happens while execute sql, error_msg:".$e->getMessage());
	$payLog->writeError("unknown exception happens while execute action.".$e->getTraceAsString());
}
print_r($Res);
/**
