...
$MEMCACHE_SERVERS = $GLOBALS['cache_servers'];
...
function menu_entry($ob,$title) {
	global $PHP_SELF;
	if($ob == 'home'){
		return '<li><a class="active" href="admincp.php">'. $title .'</a></li>';
	}
	if ($ob==$_GET['op']){
...
$PHP_SELF= 'admincp.php?mod=system&act=memcache';
$time = time();
// sanitize _GET
...
EOB;
        foreach($MEMCACHE_SERVERS as $server){
    		...
    		echo '<tr class=tr-1><td class=td-0>Current Items</td><td>',$memcacheStatsSingle[$server]['STAT']['curr_items'],'</td></tr>';
    		echo '<tr class=tr-0><td class=td-0>Total Items</td><td>',$memcacheStatsSingle[$server]['STAT']['total_items'],'</td></tr>';
    		...
    		echo '<tr class=tr-1><td class=td-0>Evictions</td><td>',$memcacheStatsSingle[$server]['STAT']['evictions'],'</td></tr>';
    		echo '</tbody></table>';
	   }
    echo <<<EOB
